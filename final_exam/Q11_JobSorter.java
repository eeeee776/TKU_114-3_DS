...
public class Q11_JobSorter {
    ...
    private static void mergeSort(Q11_Job[] data, Q11_Job[] temp, int left, int right) {
        if (left < right) {
    ...
    private static void merge(Q11_Job[] data, Q11_Job[] temp, int left, int mid, int right) {
    ...
    public static Q11_Job findFirstByOwner(Q11_Job[] jobs, String owner) {
        if (jobs == null || owner == null || owner.trim().isEmpty()) return null;
        String targetOwner = owner.trim();
        for (Q11_Job job : jobs) {
            if (job.getOwner().equalsIgnoreCase(targetOwner)) return job;
        }
        return null;
    }
}